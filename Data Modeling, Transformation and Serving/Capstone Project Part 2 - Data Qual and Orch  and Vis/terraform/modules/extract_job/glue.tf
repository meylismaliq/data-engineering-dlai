...
resource "aws_glue_job" "api_users_ingestion_etl_job" {
  ...
  default_arguments = {
    ...
    # If you are redoing the lab, the original structure was "http://<API-ENDPOINT>/users"
    "--api_url"             = "http://ec2-52-204-126-103.compute-1.amazonaws.com/users"
...
resource "aws_glue_job" "api_sessions_ingestion_etl_job" {
  ...
  default_arguments = {
    ...
    "--api_end_date"        = "2020-01-31"
    # Replace the placeholder <API-ENDPOINT> with the value from the CloudFormation outputs
    # If you are redoing the lab, the original structure was "http://<API-ENDPOINT>/sessions"
    "--api_url"             = "http://ec2-52-204-126-103.compute-1.amazonaws.com/sessions"
