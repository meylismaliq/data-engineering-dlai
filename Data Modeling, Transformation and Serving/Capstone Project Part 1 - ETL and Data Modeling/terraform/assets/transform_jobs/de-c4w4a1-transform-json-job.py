...
s3_path_users = f"{args['users_source_path']}/{ingest_day.strftime('%Y-%m-%d')}"
s3_path_sessions = f"{args['sessions_source_path']}/{ingest_day.strftime('%Y-%m-%d')}"

# Read JSON data from S3
# In this Glue job, the datasets are created directly as Spark Dataframes. Use the `s3_path_users` and `s3_path_sessions` as argument inside 
...
users_df = users_df.withColumn(
    "ingest_on", to_date(lit(str(ingest_day)), "yyyy-MM-dd")
)
